...
/** Alle attachment-ID's uit cm_galerijfotos (array of geserialiseerd). */
function cm_gallery_attachment_ids(array $meta): array
{
    if (!isset($meta['cm_galerijfotos'])) {
        return [];
    }
...
foreach ($users as $u) {
    ...
    $galerijExports = [];
    foreach (cm_gallery_attachment_ids($meta) as $gid) {
        $gu = cm_best_attachment_url($gid);
        if ($gu !== '') {
            $galerijExports[] = ['id' => $gid, 'url' => $gu];
        }
    }
