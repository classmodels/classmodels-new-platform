...
exports.CM_EMAIL_CONTENT_WIDTH = void 0;
exports.normalizeEmailContentAlignment = normalizeEmailContentAlignment;
...
function normalizeEmailContentAlignment(html) {
    return html
...
function extractEmailBodyContent(html) {
    const t = html.trim();
...
function coerceOutgoingEmailHtml(inner) {
    const t = (inner ?? '').trim();
    if (!t)
        return buildClassModelsEmailDocument('');
    if (!t.includes('<')) {
