...
function runEnsureBriefProfileMatchedSchemaSync(root) {
  const runner = path.join(root, 'scripts', 'ensure-brief-profile-matched-schema.cjs');
  const r = spawnSync(
    process.execPath,
    [
      '-e',
      `require(${JSON.stringify(runner)}).runEnsureBriefProfileMatchedSchema(${JSON.stringify(root)}).then((ok)=>process.exit(ok?0:1)).catch((e)=>{console.error(e);process.exit(1)})`,
    ],
    { cwd: root, env: process.env, stdio: 'inherit' },
  );
  return r.status === 0;
