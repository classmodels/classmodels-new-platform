...
function rebookHrefForSlug(slug: string | undefined): string {
  if (!slug || !/^[a-zA-Z0-9-]+$/.test(slug)) return '/nieuw';
  const s = slug.toLowerCase();
  if (s.includes('gratis') || s.includes('fotoshoot') || s.includes('testshoot')) {
    return '/nieuw/gasten/gratis-fotoshoot#agenda';
  }
  if (s.includes('casting')) return '/nieuw/gasten/casting#agenda';
  if (s.includes('intake')) return '/nieuw/gasten/intake#agenda';
  return '/nieuw';
}
