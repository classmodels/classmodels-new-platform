...
function buildMapsRouteBlockHtml(ctx: AgendaMailPlaceholderContext): string {
  ...
  const mapBlock = mapUrl
    ? `<p style="margin:14px 0 0;text-align:left;"><a href="${esc(mapsDir || '#')}" style="text-decoration:none;"><img src="${esc(mapUrl)}" alt="Route naar Class-Models" width="520" style="display:block;max-width:100%;height:auto;border:0;border-radius:6px;border:1px solid #e4e4e7;" /></a></p>`
    : '';
  const linkBlock = mapsDir
    ? `<p style="margin:10px 0 0;font-size:14px;text-align:left;"><a href="${esc(mapsDir)}" style="color:#6f121b;font-weight:600;">Open route in Google Maps</a></p>`
    : '';
